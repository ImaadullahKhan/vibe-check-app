    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-slate-950/80 hover:bg-slate-800 text-slate-300 hover:text-white flex items-center justify-center border border-slate-700 transition-colors"
        ...
        {/* Modal Image Header */}
        <div className="relative aspect-[16/9] w-full bg-slate-950 overflow-hidden rounded-t-3xl flex items-center justify-center">
          ...
          <div className="absolute top-4 left-4 flex flex-wrap gap-2 z-10">
            ...
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-slate-950/90 border ${
            ...
            {item.isBestseller && (
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold bg-amber-500 text-slate-950">
          ...
          <div className="absolute bottom-3 left-4 right-4 z-10">
            ...
            <h3 className="font-['Outfit',sans-serif] text-2xl sm:text-3xl font-extrabold text-white">
              {item.name}
            </h3>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-rose-400">₹{item.price}</span>
              <span className="text-xs text-slate-400">Net price (All taxes incl.)</span>
            </div>

            {item.spicyLevel && item.spicyLevel > 0 ? (
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-rose-950/60 border border-rose-800/60 text-xs font-bold text-rose-300">
                <Flame className="w-3.5 h-3.5 fill-rose-500 text-rose-500" />
                <span>
                  {item.spicyLevel === 3 ? 'Fiery Hot 🔥' : item.spicyLevel === 2 ? 'Medium Spicy 🌶️' : 'Mild'}
                </span>
              </div>
            ) : (
              <div className="text-xs text-slate-400 font-medium">Non-spicy / Sweet</div>
            )}
          </div>

          {/* Description */}
          <div>
            <h4 className="text-xs uppercase tracking-wider text-slate-400 font-bold mb-1.5">Description</h4>
            <p className="text-slate-300 text-sm leading-relaxed">{item.description}</p>
          </div>

          {/* Metadata Grid (Prep Time, Calories, Hygiene) */}
          <div className="grid grid-cols-3 gap-2.5 py-3 px-4 bg-slate-950/60 rounded-2xl border border-slate-800/80">
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 text-slate-400 text-xs mb-0.5">
                <Clock className="w-3.5 h-3.5 text-amber-400" />
                <span>Prep Time</span>
              </div>
              <span className="text-xs font-bold text-white">{item.preparationTime || '8-10 mins'}</span>
            </div>

            <div className="text-center border-x border-slate-800">
              <div className="text-slate-400 text-xs mb-0.5">Energy</div>
              <span className="text-xs font-bold text-white">{item.calories || '520 kcal'}</span>
            </div>
            ...
            <div className="text-center">
              ...
              <span className="text-xs font-bold text-emerald-400">100% Fresh</span>
            </div>
          </div>

          {/* Allergens Alert */}
          {item.allergens && item.allergens.length > 0 && (
            <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-800/40 p-3 rounded-xl border border-slate-700/40">
          ...
          <div>
            <label htmlFor="custom-instruction" className="block text-xs font-bold text-slate-300 mb-1.5">
            ...
            <input
              ...
              placeholder="e.g. Extra spicy, less mayo, pack sauce separately"
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            {/* Quantity Controller */}
            <div className="flex items-center justify-between bg-slate-950 border border-slate-800 rounded-2xl p-1 sm:w-36 shrink-0">
              <button
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-9 h-9 rounded-xl bg-slate-900 hover:bg-rose-950/60 text-slate-300 hover:text-white flex items-center justify-center transition-colors"
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-4 rounded-2xl flex items-center justify-center transition-colors shadow-lg shadow-emerald-950/50"
            >
